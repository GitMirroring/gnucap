...
{ untested();
  ...
  if (is_analog() &&  d_iter() < a_iter()) { untested();
...
{ untested();
  ...
  if (_sim->_time0 <= (final_time()-risefall)) { untested();
    return start;
...
{ untested();
  assert(in_transit());
  if (lv().is_rising()) { untested();
    set_lv(lvSTABLE1);
  }else if (lv().is_falling()) { untested();
    set_lv(lvSTABLE0);
  }else{ untested();
    // lv no change
  }
