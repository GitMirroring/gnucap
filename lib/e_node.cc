 ...
 */
NODE::NODE(const NODE& p) : CKT_BASE(p)
{ untested();
  unreachable();
}
/*--------------------------------------------------------------------------*/
...
void NODE_P::new_node(const std::string& node_name, const CARD* d)
{
  //assert(!_nnn); //BUG// fails on MUTUAL_L::expand after clone
  assert(d);
 ...
 */
NODE_P& NODE_P::new_model_node(const std::string& node_name, CARD* d)
{
