...
DISPATCHER<CMD>::INSTALL d1(&device_dispatcher, "connectrules", &p1);
/*--------------------------------------------------------------------------*/
void CONNECT_RULES::expand()
{
  ...
  for(auto p : discipline_dispatcher) {
    std::string name = p.first;
    DISCIPLINE* disc = prechecked_cast<DISCIPLINE*>(p.second);
    if(disc) {
      disc->set_owner(this);
      int id = disc->user_number();
      disc->clear();
  ...
  for(auto card : *subckt()){
	...
	unreachable();
      ...
      NODE const* n = discipline_dispatcher[connect];
      assert(n);
      int i1 = n->user_number();
      n = discipline_dispatcher[connect];
      assert(n);
