...
private: // simulation stuff
  ...
  void      tr_iwant_matrix(){ untested(); _comp->tr_iwant_matrix(); }
  double    tr_probe_num(const std::string&s)const{ untested();
    return _comp->tr_probe_num(s);
  }
  void ac_iwant_matrix(){ untested(); _comp->ac_iwant_matrix(); }
  void tr_begin()	{untested(); assert(_comp); _comp->tr_begin();}
  void tr_restore()	{untested(); assert(_comp); _comp->tr_restore();}
  void dc_advance()	{untested(); assert(_comp); _comp->dc_advance();}
  ...
  void tr_queue_eval() {untested(); assert(_comp); _comp->tr_queue_eval();}
  bool do_tr() {untested(); assert(_comp);set_converged(_comp->do_tr());return converged();}
  void tr_load()	{untested(); assert(_comp); _comp->tr_load();}
  TIME_PAIR tr_review()	{untested(); assert(_comp); return _time_by = _comp->tr_review();}
  void tr_accept() {untested(); assert(_comp); _comp->tr_accept();}
