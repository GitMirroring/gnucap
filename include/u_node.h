...
public:
  ...
  node_t&     map(); // e_compon.cc:COMPONENT::map_nodes:522
...
public:
  double      v0()const {
   ...
   // assert(!m_() || n_()->m_() == m_());
    ...
    return CKT_BASE::_sim->_v0[m_()];
  }
  
  COMPLEX     vac()const {
    assert(m_() >= 0);
    assert(m_() <= CKT_BASE::_sim->_total_nodes);
    assert(n_());
    //assert(n_()->m_() == m_());
    //assert(n_()->vac() == NODE::_ac[m_()]);
    return CKT_BASE::_sim->_ac[m_()];
  }
  
  double&     i() {
    assert(m_() >= 0);
    assert(m_() <= CKT_BASE::_sim->_total_nodes);
    return CKT_BASE::_sim->_i[m_()];
  }
#if 0
