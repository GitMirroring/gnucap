...
template <class T>
void BSMATRIX_SOLVER<T>::dezero(T const& offset)
{
...
private:
  ...
  using BSMATRIX_SOLVER<T>::_data; // BUG
...
private: // solver overrides
  ...
  void dezero(T const& x)override;
...
/*--------------------------------------------------------------------------*/
template <class T>
void LU_COPY<T>::dezero(T const& offset)
{
  for (int ii = 1;  ii <= size();  ++ii) {
    d_(_lu, ii) += offset;
  }
  BSMATRIX_SOLVER<T>::dezero(offset);
}
/*--------------------------------------------------------------------------*/
template <class T>
void LU_COPY<T>::lu_decomp(bool do_partial)
{
  ...
  for (int mm = 1;   mm <= size();   ++mm) {
	...
	{ /* jj == mm */
	  ...
	  if (subtract_dot_product(mm,mm,mm,d_(aa,mm)) == 0.) {itested();
