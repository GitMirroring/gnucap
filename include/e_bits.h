...
protected: // NODE_P access
  ...
  uint16_t type()const { return uint16_t(_uint << 1 >> 49); }
  int number()const { return uint32_t(_uint >> 16); }
...
protected:
  ...
  int number()const { assert(is_number()); return _number; }
  NODE* node()const { assert(is_node() || is_none()); return _nnn; }
  NODE* node_safe()const { return node(); }
  NODE_P* next() { assert(is_link()); return _link; }
  NODE_P* next() const { assert(is_link()); return _link; }
