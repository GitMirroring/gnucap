/*                       -*- C++ -*-
 * Copyright (C) 2022 Felix Salfelder
 *
...
#define PORTS_PER_SUBCKT 100 // BUG
/*--------------------------------------------------------------------------*/
...
private:
  bool is_device() const override { return !at_top_level(); }
  ...
  // void set_port_by_name(std::string& name, std::string& value)override{ untested();
  //   assert(_dev);
  ...
  //   assert(_dev);
  //   trace3("PARAMSET::spbi", long_label(), index, value);
...
private:
  bool at_top_level() const { return !owner(); }
...
#if 1
  void precalc_last() override { assert(at_top_level());}
  void tr_begin() override{ assert(at_top_level());}
  void ac_begin() override{ assert(at_top_level());}
  void tr_load() override{ assert(at_top_level());}
  bool tr_needs_eval()const override{ untested(); assert(at_top_level()); return false;}
  void tr_queue_eval()override{ assert(at_top_level());}
  void tr_accept() override{ assert(at_top_level());}
  double tr_probe_num(const std::string&)const override{untested(); return NOT_VALID;}
  void tr_advance() override{ assert(at_top_level());}
  void dc_advance() override{ assert(at_top_level());}
  bool do_tr() override{ untested(); assert(at_top_level()); return true;}
  void do_ac() override{ assert(at_top_level());}
  void ac_load() override{ assert(at_top_level());}
#else
...
{
  assert(scope());
  trace5("PARAMSET::pf", long_label(), owner(), _dev, _sim->is_first_expand(), at_top_level());
